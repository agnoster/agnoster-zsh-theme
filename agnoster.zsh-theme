...
check_envs() {
  local cnd=`[[ -n "$CONDA_DEFAULT_ENV" ]] && [[ "$CONDA_DEFAULT_ENV" == "base" ]] && echo -n "0" || echo -n "1"`
  local rb=`[[ -n "$RBENV_VERSION" ]] && [[ "$RBENV_VERSION" == "system" ]] && echo -n "0" || echo -n "1"`
  local nde=`[[ -n "$NODENV_VERSION" ]] && [[ "$NODENV_VERSION" == "system" ]] && echo -n "0" || echo -n "1"`
...
promtpt_all_virtual_env() {
  ...
  elif [[ -n $AGNOSTER_THEME_TRIM ]] && [[ $multiple_envs == 1 ]]; then
    prompt_nodenv 1
    prompt_rbenv 1
    prompt_condaenv 1
    prompt_virtualenv
